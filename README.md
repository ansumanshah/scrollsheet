...
One primitive for bottom sheets, drawers, modal dialogs, side panels, and toasts. A real `<dialog>` in the top layer. The browser's own scroll engine for gestures. Spring physics compiled to CSS `linear()`. <!--size:index.gzip:1-->18.1<!--/size--> kB gzipped, <!--size:index.brotli:1-->16.1<!--/size--> kB brotli, plus a mandatory 3.8 kB stylesheet: 22.3 kB combined. React 18+.
...
- **Zero-config entry.** `scrollsheet/auto` embeds the stylesheet and injects it on first open: no CSS import needed, <!--size:auto.gzip:1-->22.1<!--/size--> kB gzip for `Sheet` against the default entry's <!--size:index.gzip:1-->18.1<!--/size-->.
