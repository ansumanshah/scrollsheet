...
`document.body`, positioned with CSS instead of stacked inside a shared sheet panel. Every
method (`toast()`, `.success()`,
`.error()`, `.info()`, `.warning()`, `.loading()`, `.message()`, `.custom(jsx)`, `.promise()`,
`.dismiss(id?)`) works before or after a `<Toaster>` mounts, backed by one shared
module-level queue.
...
to the `Toaster` instead: `scrollsheet/styles.css` and `scrollsheet/toast.css`. For sonner's
zero-setup behavior, import `toast` and `Toaster` from `scrollsheet/auto`: embedded styles,
injected on first render. The `auto` entry is also the right one for design systems whose CSS
pipeline statically compiles custom properties away (postcss-css-variables and similar); that
compilation destroys the runtime variables the stylesheets' geometry runs on.
