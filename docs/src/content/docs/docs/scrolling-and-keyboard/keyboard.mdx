...
The panel overdraws about 120vh past its bottom edge, so a stale frame never shows a gap while
the keyboard pushes the content up.
...
The gate is a measured nonzero inset, never focus alone: a desktop click into an input expands
nothing.
